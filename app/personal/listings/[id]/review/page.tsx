  ...
  async function review(changeId: string, status: 'approved' | 'rejected', note?: string) {
    ...
    if (res.ok) {
      setChanges(prev =>
        prev.map(c =>
          c.id === changeId
            ? { ...c, status, agent_note: note ?? null, reviewed_at: new Date() }
            : c
        )
      )
    }
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <Link href="/personal" className="text-sm text-gray-400 hover:text-gray-600">← הנכסים שלי</Link>
          <h1 className="text-xl font-bold text-gray-900 mt-1">שינויים ממוכר</h1>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <input
            type="email"
            placeholder="מייל לשליחה (אופציונלי)"
            value={sellerEmail}
            onChange={e => setSellerEmail(e.target.value)}
            className="text-sm border border-gray-300 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 w-44"
            dir="ltr"
          />
          <button
          ...
          >
            {sendingLink ? 'יוצר...' : 'צור קישור למוכר'}
          </button>
        </div>
      </div>
      ...
      {sellerUrl && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-4 text-sm space-y-2">
          <p className="font-medium text-green-800">קישור נוצר — תקף 7 ימים</p>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-gray-400">
          ...
          <p className="text-sm">אין שינויים ממתינים. צור קישור למוכר כדי לאפשר לו להציע עדכונים.</p>
        </div>
      ) : (
