    ...
    return (
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {images.map((img) => (
          ...
          <img
            key={img.id}
            src={img.enhancedBlobUrl ?? img.enhancedDataUrl ?? img.blobUrl ?? img.dataUrl}
            alt={img.name}
        ...
        >
          {heroImage && (
            <>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src={heroImage.enhancedBlobUrl ?? heroImage.enhancedDataUrl ?? heroImage.blobUrl ?? heroImage.dataUrl}
                alt={title}
